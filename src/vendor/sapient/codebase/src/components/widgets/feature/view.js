(function() {
  ...
  function featureViewController($scope, $q, $interval, featureData) {
   ...
   $scope.formatNumber = function(i) {
            return ((parseFloat(i) || "") === "")?"":parseFloat(i).toFixed(1).replace(/(?:\.0+|(\.\d+?)0+)$/, "$1");
        }
    /**
    ...
    ctrl.load = function() {
      var deferred = $q.all([
        ...
        featureData.projectSprintDetailRetrieves({componentId: $scope.widgetConfig.componentId}).then(function(data) { processSprintStatusResponse(data);}),
        featureData.ftprReopen({}).then(function(data) { ftprReopenResponse(data) })
    ...
    function processSprintStatusResponse(data){
      ctrl.showStatusStories = data;
    }

    function ftprReopenResponse(data){

        var ftprLabels = [], reopenRateLabels = [],
            ftprSeries = [], reopenRateSeries = [];

         _.forEach(data.ftpr, function (val, key) {
            ftprLabels.push(key);
        ...
        ctrl.reopenRateData = {
                    labels: reopenRateLabels,
                    series: [reopenRateSeries],
