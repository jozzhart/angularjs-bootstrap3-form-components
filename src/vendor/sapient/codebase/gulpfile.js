...
var browserSync = require('browser-sync'),
  ...
  jsFiles = [
    'src/{app,components,speedy-components/dist}/**/*.js'
  ],
...
gulp.task('build', function (callback) {
  runSequence('clean', ['assets', 'themes', 'fonts', 'js', 'views', 'test-data', 'helpers', 'logo'], 'html', callback);
});
...
gulp.task('helpers', function () {
  return gulp
    ...
    .pipe(gulp.dest(hygieia.dist + 'helpers'));
});
...
//move logo from root folder
gulp.task('logo', function () {
  return gulp
    .src([
      'logo/*'
    ])
    .pipe(gulp.dest(hygieia.dist + 'logo'));
});
...
gulp.task('sonar', function () {
  var options = {
    sonar: {
      host: {
        url: 'https://tools.publicis.sapient.com/sonar'
      },
      projectKey: 'sonar:my-project:1.0.0',
      projectName: 'SR - DevOps Dashboard - UI',
      projectVersion: '1.0.0',
      // comma-delimited string of source directories 
      sources: 'src/components/templates/custom.js',
      language: 'js',
...
gulp.task('e2e', ['mongorestore'], function () {
