...
(function () {
    ...
    angular
        ...
        .controller('SecurityWidgetViewController', SecurityWidgetViewController);

    SecurityWidgetViewController.$inject = ['$scope', 'securityData', 'DisplayState', '$q', '$uibModal'];
    function SecurityWidgetViewController($scope, securityData, DisplayState, $q, $uibModal) {
        var ctrl = this;
        var builds = [];
        ...
        ctrl.load = function() {
            ...
            securityData.details(params).then(function(data) {
                builds = data.result;
                processResponse(builds);
                deferred.resolve(data.lastUpdated);
            });
            securityData.securityIssues({}).then(function(data) {
               processSecurityIssues(data);
            });
        ...
        ctrl.detail = function(build) {
            $uibModal.open({
                templateUrl: 'components/widgets/security/detail.html',
                controller: 'SecurityWidgetDetailController',
                controllerAs: 'detail',
                size: 'lg',
                resolve: {
                    build: function() {
        ...
        function processSecurityIssues(data){
        ...
        function processResponse(data) {
            var worker = {
                    averageSecurityBuildDuration: averageSecurityBuildDuration,
                    buildsSecurityPerDay: buildsSecurityPerDay,
                    latestSecurityBuilds: latestSecurityBuilds,
                    setSecurityDisplayToErrorState: setSecurityDisplayToErrorState,
                    totalSecurityBuilds: totalSecurityBuilds
                };

            //region web worker method implementations
            function averageSecurityBuildDuration(data, buildThreshold, cb) {
            ...
            function buildsSecurityPerDay(data, cb) {
            ...
            function latestSecurityBuilds(data, cb) {
            ...
            function setSecurityDisplayToErrorState(data, failureThreshold, cb) {
                // order by end time and limit to last 5
            ...
            function totalSecurityBuilds(data, cb) {
            ...
            worker.buildsSecurityPerDay(data, function (data) {
            ...
            worker.latestSecurityBuilds(data, function (buildsToDisplay) {
                //$scope.$apply(function () {
                    ctrl.recentBuilds = buildsToDisplay;
                //});
            });

            worker.averageSecurityBuildDuration(data, $scope.widgetConfig.options.buildDurationThreshold, function (buildDurationData) {
                //$scope.$apply(function () {
                var labels = [];
            ...
            worker.setSecurityDisplayToErrorState(data, $scope.widgetConfig.options.consecutiveFailureThreshold, function (displayAsErrorState) {
                //$scope.$apply(function () {
                    $scope.display = displayAsErrorState ? DisplayState.ERROR : DisplayState.DEFAULT;
                //});
            });

            worker.totalSecurityBuilds(data, function (data) {
                //$scope.$apply(function () {
