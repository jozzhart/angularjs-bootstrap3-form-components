...
(function () {
    ...
    function BuildWidgetViewController($scope, buildData, DisplayState, $q, $uibModal) {
        ...
        ctrl.load = function() {
            ...
            var collectorItemIdField="";
            var collectorItemId="";
            if($('.build-nav-tabs li:hover').text()){
              collectorItemIdField=$('.build-nav-tabs li:hover').text().trim();
            }else if($('.build-nav-tabs li.active').text()){
              collectorItemIdField=$('.build-nav-tabs li.active').text().trim();
            }
            for(var t=0;t<$scope.dashboard.application.components[0].collectorItems.Build.length;t++){
                   if(collectorItemIdField==$scope.dashboard.application.components[0].collectorItems.Build[t].description){
                     collectorItemId=$scope.dashboard.application.components[0].collectorItems.Build[t].id;
              }
            }
            ...
            if(collectorItemId){
            params.collectorItemId=collectorItemId;
            }
        ...
        function processResponse(data) {
            ...
            function lastSuccessfulbuild(data, cb) {
            	var result='NA';
            ...
            //endregion
            worker.lastSuccessfulbuild(data, function (data) {
            	ctrl.lastSuccessfulBuildNumber = data.lastSuccess;
            }); 
