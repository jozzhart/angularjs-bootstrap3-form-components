...
(function () {
    ...
    angular
        ...
        .controller('SecurityWidgetConfigController', SecurityWidgetConfigController);
    SecurityWidgetConfigController.$inject = ['modalData', '$scope', 'collectorData', '$uibModalInstance'];
    function SecurityWidgetConfigController(modalData, $scope, collectorData, $uibModalInstance) {
        var ctrl = this,
        ...
        $scope.getJobs = function (filter) {
        	return collectorData.itemsByType('build/searchCollectorItem', {"search": "ZAP", "size": 20}).then(function (response){
        		return response;
        	});
        }
        ...
        function submitForm(valid, collector) {
            if (valid) {
                var form = document.buildSecurityConfigForm;
                var postObj = {
                    name: 'security',
