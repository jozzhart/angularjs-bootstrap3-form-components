(function () {
    ...
    function performanceViewController($q, $scope, performanceData, $uibModal, collectorData) {
        ...
        ctrl.load = function() {
            ...
            performanceData.paDefectListDetails({componentId: $scope.widgetConfig.componentId}).then(function(data) {
                    defectListResponse(data);
                    
            });
            performanceData.paPerfJobStatusDetails({componentId: $scope.widgetConfig.componentId}).then(function(data) {
                    perfJobStatusResponse(data);
            });
        ...
        function defectListResponse(data){
             var labels = [],
             seriesFrontend=[],
             seriesBackend = [];

             _(data.PerformanceDefectList).forEach(function(d) {
                    
                    labels.push('');

                    seriesFrontend.push({meta: d.defectCreationDate +" - "+ d.frontendDefectCount, value: d.frontendDefectCount});
                    seriesBackend.push({meta: d.defectCreationDate +" - "+ d.backendDefectCount, value: d.backendDefectCount});
                });

            ctrl.defectListData = {
                    labels: labels,
                    series: [seriesFrontend,seriesBackend],

                };
                
        }

        function perfJobStatusResponse(data){
				ctrl.frontEndJobName = data.frontendJobName;
				ctrl.backendJobName = data.backendJobName;
                ctrl.perfJobStatusData = data.performanceJobData;
        }
