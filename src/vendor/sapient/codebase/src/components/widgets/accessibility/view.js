...
(function () {
    ...
    angular
        ...
        .controller('AccessibilityWidgetViewController', AccessibilityWidgetViewController);

    AccessibilityWidgetViewController.$inject = ['$scope', 'securityData', 'DisplayState', '$q', '$uibModal'];
    function AccessibilityWidgetViewController($scope, accessibilityData, DisplayState, $q, $uibModal) {
        var ctrl = this;
        var builds = [];
        ...
        ctrl.load = function() {
            ...
            var params = {
                ...
                numberOfDays: 15,
                buildJobCollectorItemId: $scope.widgetConfig.collectorItemIds[0]
            };
            accessibilityData.details(params).then(function(data) {
                builds = data.result;
                processResponse(builds);
                deferred.resolve(data.lastUpdated);
            });
            accessibilityData.accessibilityIssues(params).then(function(data) {
               processAccessibilityIssues(data);
            });
        ...
        ctrl.detail = function(build) {
            $uibModal.open({
                templateUrl: 'components/widgets/accessibility/detail.html',
                controller: 'AccessibilityWidgetDetailController',
                controllerAs: 'detail',
                size: 'lg',
                resolve: {
                    build: function() {
        ...
        function processAccessibilityIssues(data){
            var dataCountMap = data.dataCountMap,
            byStatus = [];
            ctrl.ByPriority = _.sortBy(dataCountMap.ByPriority, 'priority');

            _.forEach(dataCountMap.ByStatus, function (val,key) {
                if(val.sStatus === "In Progress"){
                    byStatus['wip'] = val;
                }else{
                    byStatus[val.sStatus] = val;
                }
            });

            ctrl.ByStatus = byStatus;
        ...
        function processResponse(data) {
            var worker = {
                    averageAccessibilityBuildDuration: averageAccessibilityBuildDuration,
                    buildsAccessibilityPerDay: buildsAccessibilityPerDay,
                    latestAccessibilityBuilds: latestAccessibilityBuilds,
                    setAccessibilityDisplayToErrorState: setAccessibilityDisplayToErrorState,
                    totalAccessibilityBuilds: totalAccessibilityBuilds
                };

            //region web worker method implementations
            function averageAccessibilityBuildDuration(data, buildThreshold, cb) {
            ...
            function buildsAccessibilityPerDay(data, cb) {
            ...
            function latestAccessibilityBuilds(data, cb) {
            ...
            function setAccessibilityDisplayToErrorState(data, failureThreshold, cb) {
                // order by end time and limit to last 5
            ...
            function totalAccessibilityBuilds(data, cb) {
            ...
            worker.buildsAccessibilityPerDay(data, function (data) {
            ...
            worker.latestAccessibilityBuilds(data, function (buildsToDisplay) {
                //$scope.$apply(function () {
                    ctrl.recentBuilds = buildsToDisplay;
                //});
            });

            worker.averageAccessibilityBuildDuration(data, $scope.widgetConfig.options.buildDurationThreshold, function (buildDurationData) {
                //$scope.$apply(function () {
                var labels = [];
            ...
            worker.setAccessibilityDisplayToErrorState(data, $scope.widgetConfig.options.consecutiveFailureThreshold, function (displayAsErrorState) {
                //$scope.$apply(function () {
                    $scope.display = displayAsErrorState ? DisplayState.ERROR : DisplayState.DEFAULT;
                //});
            });

            worker.totalAccessibilityBuilds(data, function (data) {
                //$scope.$apply(function () {
