...
(function () {
    ...
    angular
        ...
        .controller('AddNewBuildWidgetConfigController', AddNewBuildWidgetConfigController);
    AddNewBuildWidgetConfigController.$inject = ['modalData', '$scope', 'collectorData', '$uibModalInstance'];
    function AddNewBuildWidgetConfigController(modalData, $scope, collectorData, $uibModalInstance) {
        var ctrl = this,
        ...
        function submitForm(valid, collector) {
            if (valid) {
            	var newId=widgetConfig.options.id.replace("addnewbuild", "build");
            	widgetConfig.options.id=newId;
                var form = document.addNewBuildConfigForm;
                var postObj = {
                    name: 'build',
                    options: {
                ...
                setTimeout(function(){ if($('.build-nav-tabs li:last').text().trim()=='Add New'){
            		$('.build-nav-tabs li:last').click();
            	}else{
        	 	$('.build-nav-tabs li:last').click(); } }, 3000);
            }
        }
    }
})();
