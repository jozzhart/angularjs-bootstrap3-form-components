(function () {
    ...
    function CodeAnalysisViewController($scope, codeAnalysisData, testSuiteData, libraryPolicyData, $q, $filter, $uibModal) {
        var ctrl = this;
        ctrl.automationPercentage = {};
        
        ctrl.regressionPassPercentage={};
        ctrl.reviewCoverage = {};
        ctrl.reviewCoverageOpen = {};
        ...
        ctrl.regressionPassPercentageOption = {
                    fullWidth: true,
                    height: "200px",
                    low:0,
                    plugins: [
        ...
        ctrl.load = function () {
            ...
            var liRequest = {
             ...
             processLeakageResponse();
            
            var defectFixRequest = {
            	componentId: $scope.widgetConfig.componentId
            };
            ...
            return $q.all([
                ...
                codeAnalysisData.leakageInfoDetails(liRequest).then(processLeakageResponse),
                codeAnalysisData.defectRejectionRateDetails(liRequest).then(defectRejectionRateResponse),
                codeAnalysisData.fetchDefectFixRateDetails(defectFixRequest).then(processDefectFixrate),
                codeAnalysisData.fetchAutomationPercentageDetails(defectFixRequest).then(processAutomationPercentage),
 				codeAnalysisData.fetchReviewCoverageDetails(defectFixRequest).then(processReviewCoverage),
                codeAnalysisData.fetchRegressionPassPercentageDetails(defectFixRequest).then(processRegressionPassPercentage),
                codeAnalysisData.fetchReviewCoverageDetails(openReviewCov).then(processReviewMergedCoverage)
        ...
        $scope.formatNumber = function(i) {
            return ((parseFloat(i) || "") === "")?0:parseFloat(i).toFixed(1).replace(/(?:\.0+|(\.\d+?)0+)$/, "$1");
        }
        function processReviewMergedCoverage(res){
        	ctrl.reviewCoverage["open"] = res;
        }
        function processReviewCoverage(res){
            ctrl.reviewCoverage["merged"] = res;
           
        }
        function processAutomationPercentage(res){
        	ctrl.automationPercentage = res;
        }

        function processRegressionPassPercentage(res){
        	var labels = [],series=[], buildDurationData = {};
            _(res.RegressionPassPercent).forEach(function(val,key) {
                
                labels.push(key);

                series.push({meta: key+" in " + val + "%",value: val});
            });
            labels.push('');
        ...
        function defectRejectionRateResponse(res){
           
                
                var labels = [],series=[], buildDurationData = {};
                _(res).forEach(function(d) {
                    
                    labels.push(d.sprintName);

                    series.push({meta: d.defectRejectionPercentage+"% in " + d.sprintName,value: d.defectRejectionPercentage});
                });
                labels.push('');

                ctrl.defectRejectionData = {
                    labels: labels,
                    series: [series],
        ...
        function processLeakageResponse(res){
                ...
                _(res).forEach(function(d) {
                    ...
                    series.push(createLeakageSer(d));
                });
                labels.push('');
        ...
        function createLeakageSer(response){
            return {
                    meta: response.leakagePercentage+"% in " + response.sprintName,
                    value: response.leakagePercentage
                    };
        }
        function processDefectFixrate(response){
            var series = [];
              _(response.data).forEach(function(d,k) {
                    series.push({meta: response.labels[k] +" : ("+ d + ")", value: d});
                });

             ctrl.defectFixRateData = {
                    labels: response.labels,
                    series: [series]
                };
            ctrl.defectFixRateLavel = response.data.length;
        }
